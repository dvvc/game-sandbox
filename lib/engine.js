...
function initEnv(opts) {
  ...
  // If we are using an offscreen canvas, create it
  let offscreenCtx;
...
function initGame(state, env) {
  ...
  // If we are drawing to the offscreen canvas, replace the `ctx` object with the `offscreenCtx`
  let mainCtx;
  if (env.offscreenCtx) {
    mainCtx = env.ctx;
  ...
  (function gameLoop(time) {
    ...
    // If we are drawing to offscreen context, draw the offscreen contents to the main canvas
    if (env.offscreenCtx) {
